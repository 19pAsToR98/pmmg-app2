    ...
    <div className="flex flex-col h-full bg-theme-khaki overflow-hidden">
      <header className="sticky top-0 z-50 bg-theme-navy px-4 py-4 flex items-center justify-between shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-theme-yellow tracking-wider uppercase mt-1">Relatório Tático Assistido</p>
          </div>
        </div>
        <span className="material-symbols-outlined text-theme-yellow fill-icon">mic</span>
      </header>
      ...
      <main className="flex-1 overflow-y-auto pb-4 no-scrollbar px-4 pt-6 flex flex-col items-center">
        ...
        <div className="flex flex-col items-center pt-4 px-6">
          ...
          <div className="relative bg-white rounded-[1.5rem] p-4 px-6 shadow-lg mb-12 text-center border-2 border-theme-navy/5 after:content-[''] after:absolute after:top-0 after:left-1/2 after:w-0 after:h-0 after:border-[20px] after:border-transparent after:border-b-white after:mt-[-20px] after:ml-[-20px] after:border-t-0">
            <p className="text-theme-navy font-semibold text-sm">
              Pressione o microfone para iniciar o relato. O tempo máximo de gravação é de <span className="font-bold text-theme-red">2 minutos</span>.
            </p>
          </div>
        </div>
        ...
        <section className="flex flex-col items-center justify-center flex-1 w-full max-w-xs">
          ...
          <div className={`w-full text-center mb-8 transition-opacity duration-500 ${isRecording ? 'opacity-100' : 'opacity-50'}`}>
            <p className="text-theme-navy font-black text-4xl tracking-widest">
              {formatTime(duration)}
            </p>
            <p className="text-[10px] font-bold uppercase text-theme-navy/60 mt-1">
          ...
          <button 
            ...
            className={`w-32 h-32 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 ${
              isRecording 
                ? 'bg-theme-red scale-110 ring-8 ring-theme-red/30' 
                : 'bg-theme-navy scale-100 ring-0'
            }`}
          >
