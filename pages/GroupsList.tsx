...
const GroupsList: React.FC<GroupsListProps> = ({ navigateTo, userGroups, officers, openGroup, pendingRequestsCount }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-theme-khaki overflow-hidden">
      <header className="sticky top-0 z-50 bg-theme-navy text-white shadow-xl px-4 py-4 shrink-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 shrink-0 bg-white rounded-full flex items-center justify-center p-1.5 border-2 border-theme-red shadow-inner">
              <span className="material-symbols-outlined text-theme-navy text-xl">folder_shared</span>
            </div>
            <div>
              <h1 className="font-bold text-sm leading-none uppercase tracking-widest">Inteligência</h1>
              <p className="text-[10px] font-medium text-theme-yellow tracking-wider uppercase mt-1">Grupos de Monitoramento</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {pendingRequestsCount > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 bg-theme-red text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-theme-navy">
            ...
            <button 
              ...
              className="w-10 h-10 flex items-center justify-center rounded-xl bg-theme-yellow text-theme-navy shadow-lg active:scale-95 transition-transform"
        ...
        <div className="flex gap-2 bg-black/20 p-1.5 rounded-2xl">
           <div className="flex-1 relative">
             <span className="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-white/40 text-sm">key</span>
             <input 
               value={inviteCode}
               onChange={(e) => setInviteCode(e.target.value)}
               placeholder="CÓDIGO DE CONVITE"
               className="w-full bg-white/5 border-none rounded-xl py-2 pl-9 pr-3 text-[10px] font-black text-white placeholder:text-white/20 focus:ring-1 focus:ring-theme-yellow"
             />
           </div>
           <button 
             onClick={() => alert(`Tentando entrar no grupo com código: ${inviteCode}`)}
             className="bg-theme-yellow text-theme-navy px-4 py-2 rounded-xl text-[9px] font-black uppercase shadow-md active:scale-95 transition-transform"
           >
             Entrar
           </button>
        </div>
      ...
      <main className="flex-1 overflow-y-auto pb-40 no-scrollbar px-4 pt-4">
        <div className="flex items-center gap-2 mb-4 px-1">
          ...
          <h3 className="text-[10px] font-black text-theme-navy/60 uppercase tracking-widest">Meus Grupos de Operação</h3>
        </div>
          ...
          {userGroups.map(group => {
            ...
            return (
              <div 
                ...
                className="pmmg-card p-4 flex flex-col gap-3 active:scale-[0.98] transition-all shadow-md border-l-4 border-l-theme-navy"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-theme-navy/5 flex items-center justify-center text-theme-navy">
                      <span className="material-symbols-outlined text-3xl">folder_shared</span>
                    </div>
                    <div>
                      <h4 className="text-sm font-black text-theme-navy uppercase leading-none">{group.name}</h4>
                      <p className="text-[9px] text-theme-navy/40 font-bold uppercase mt-1 tracking-wider">{members.length} membros ativos</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                  <div className="flex -space-x-2">
                    {members.slice(0, 3).map((m, i) => (
                      <div key={i} className="w-7 h-7 rounded-full bg-theme-navy text-theme-yellow text-[8px] font-black flex items-center justify-center border-2 border-white shadow-sm ring-1 ring-black/5 uppercase">
                        {m.name.charAt(0)}
                      </div>
                    ))}
                    {members.length > 3 && (
                  ...
                  <div className="bg-theme-navy/5 px-2 py-1 rounded text-[8px] font-black text-theme-navy/60 uppercase tracking-widest border border-theme-navy/5">
