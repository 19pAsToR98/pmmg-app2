...
const GroupsList: React.FC<GroupsListProps> = ({ navigateTo, userGroups, officers, openGroup, pendingRequestsCount }) => {
  ...
  // Componente de Card de Grupo Reutilizável
  const GroupCard: React.FC<{ group: Group }> = ({ group }) => {
    ...
    return (
      <div 
        ...
        className="pmmg-card p-4 flex flex-col gap-3 active:scale-[0.98] transition-all shadow-md border-l-4 border-l-pmmg-navy dark:border-l-pmmg-yellow cursor-pointer"
      >
  ...
  return (
    <div className="flex flex-col h-full bg-pmmg-khaki dark:bg-slate-900 overflow-hidden">
      ...
      <div className="hidden lg:flex flex-col h-full">
        ...
        <header className="sticky top-0 z-50 bg-pmmg-khaki/90 dark:bg-slate-800/90 backdrop-blur-md px-8 py-4 shadow-md border-b border-pmmg-navy/10 dark:border-slate-700 shrink-0">
          <div className="flex items-center justify-between">
            <h1 className="font-black text-xl text-primary-dark uppercase tracking-tight">Grupos de Inteligência</h1>
            
            <div className="flex items-center gap-4">
              {/* Botão para Contatos */}
              <button 
                onClick={() => navigateTo('contacts')}
                className="relative bg-white dark:bg-slate-700 p-2 rounded-xl text-pmmg-navy dark:text-white shadow-md active:scale-95 transition-transform flex items-center gap-2"
                title="Gerenciar Contatos"
              >
                <span className="material-symbols-outlined text-xl">person_add</span>
                <span className="text-xs font-bold uppercase">Contatos</span>
                {pendingRequestsCount > 0 && (
                  <span className="w-5 h-5 bg-pmmg-red text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white absolute -top-2 -right-2">
                    {pendingRequestsCount}
                  </span>
                )}
              </button>
              ...
              <button 
                onClick={() => navigateTo('groupCreation')}
                className="bg-pmmg-navy text-white p-2 rounded-xl shadow-md active:scale-95 transition-transform flex items-center gap-2"
                title="Criar Novo Grupo"
              >
                <span className="material-symbols-outlined text-xl text-pmmg-yellow">add</span>
                <span className="text-xs font-bold uppercase">Novo Grupo</span>
              </button>
            </div>
          </div>
          
          {/* Barra de Convite */}
          <div className="flex gap-3 bg-white/50 dark:bg-slate-700/50 p-2 rounded-xl mt-4 border border-pmmg-navy/10 dark:border-slate-700">
             <div className="flex-1 relative">
               <span className="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-sm text-pmmg-navy/40 dark:text-slate-400">key</span>
               <input 
                 value={inviteCode}
                 onChange={(e) => setInviteCode(e.target.value)}
                 placeholder="INSIRA O CÓDIGO DE CONVITE DO GRUPO"
                 className="w-full bg-white/80 dark:bg-slate-800 border-none rounded-lg py-2 pl-9 pr-3 text-xs font-black text-primary-dark placeholder:text-pmmg-navy/30 dark:placeholder:text-slate-500 focus:ring-1 focus:ring-pmmg-yellow dark:text-slate-200"
               />
             </div>
             <button 
               onClick={() => alert(`Tentando entrar no grupo com código: ${inviteCode}`)}
               className="bg-pmmg-yellow text-pmmg-navy px-6 py-2 rounded-lg text-xs font-black uppercase shadow-md active:scale-95 transition-transform shrink-0"
             >
               Entrar no Grupo
             </button>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-8 no-scrollbar">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-sm font-black text-pmmg-navy/80 dark:text-slate-400 uppercase tracking-widest">Meus Grupos de Operação ({userGroups.length})</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            {userGroups.length === 0 && (
              <div className="text-center py-20 opacity-30 col-span-full">
                <span className="material-symbols-outlined text-7xl">folder_off</span>
                <p className="text-lg font-black uppercase mt-4 tracking-[0.2em]">Nenhum grupo ativo</p>
                <p className="text-sm font-bold uppercase mt-1">Crie um grupo ou entre com um código</p>
              </div>
            )}
          </div>
        </main>
