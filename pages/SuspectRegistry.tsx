...
const SuspectRegistry: React.FC<SuspectRegistryProps> = ({ navigateTo, onSave, onUpdate, currentSuspect, allSuspects }) => {
  ...
  const getMiniMapIcon = (isApproach: boolean) => {
    ...
    // Usando variáveis CSS para cores
    const color = 'var(--color-navy)';
    ...
    const svg = `
      <svg width="32" height="32" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
        <circle cx="16" cy="16" r="14" fill="${color}" stroke="var(--color-yellow)" stroke-width="2"/>
        <path d="${pathData}" fill="var(--color-yellow)" transform="translate(6 6) scale(0.75)"/>
      </svg>
    `;
  ...
  return (
    <div className="flex flex-col h-full bg-theme-khaki overflow-hidden">
      <header className="sticky top-0 z-50 bg-theme-navy px-4 py-4 flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-theme-yellow tracking-wider uppercase mt-1">SISTEMA OPERACIONAL</p>
          </div>
        </div>
        <button 
          onClick={triggerFileInput}
          className="bg-theme-navy border border-theme-yellow/30 p-1.5 rounded-md text-theme-yellow active:scale-95 transition-transform"
        >
      ...
      <main className="flex-1 overflow-y-auto pb-32 no-scrollbar px-4 pt-6">
            ...
            <>
              {photos.map((src, idx) => (
                <div key={idx} className="relative shrink-0">
                  <div className="w-32 h-44 bg-white rounded-xl overflow-hidden border-2 border-theme-navy shadow-lg">
                    <img src={src} alt={`Preview ${idx}`} className="w-full h-full object-cover" />
                    {/* Botão de exclusão movido para dentro do contêiner da imagem, posicionado no canto superior direito */}
                    <button 
                      onClick={() => removePhoto(idx)}
                      className="absolute top-1 right-1 bg-theme-red text-white w-5 h-5 rounded-full flex items-center justify-center shadow-md border border-white"
                    >
                      <span className="material-symbols-outlined text-xs">close</span>
                    </button>
                    {idx === 0 && (
                      <div className="absolute bottom-2 left-2 bg-theme-navy text-white text-[8px] font-bold px-2 py-0.5 rounded uppercase tracking-widest border border-white/20">Principal</div>
                    )}
                  </div>
                </div>
              ))}
              <button 
                onClick={triggerFileInput}
                className="w-32 h-44 bg-white/30 border-2 border-dashed border-theme-navy/30 rounded-xl flex flex-col items-center justify-center shrink-0 active:bg-white/50 transition-colors"
              >
                <span className="material-symbols-outlined text-theme-navy/40 text-3xl">add</span>
                <span className="text-[9px] font-bold text-theme-navy/60 uppercase mt-1">Mais Fotos</span>
              </button>
            </>
          ) : (
            <div 
              onClick={triggerFileInput}
              className="w-full h-52 bg-white/50 border-2 border-dashed border-theme-navy/30 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden group cursor-pointer hover:border-theme-navy/50 transition-colors shadow-inner"
            >
              <div className="w-16 h-16 rounded-full bg-theme-navy/5 flex items-center justify-center mb-2">
                <span className="material-symbols-outlined text-theme-navy/40 text-4xl">add_a_photo</span>
              </div>
              <span className="text-[10px] font-bold text-theme-navy/60 uppercase tracking-widest">Adicionar Fotos</span>
              <p className="text-[8px] text-theme-navy/40 mt-1 uppercase">Suporta múltiplos arquivos</p>
            </div>
          )}
        </div>

        {/* Status Section (Existing) */}
        <p className="text-[10px] font-bold uppercase text-theme-navy/70 mb-3 mt-4 text-center tracking-wider">Status de Monitoramento</p>
        <div className="grid grid-cols-4 gap-2">
          {[
            { id: 'Foragido', icon: 'warning', color: 'text-theme-red', border: 'border-theme-red/30' },
            { id: 'Suspeito', icon: 'visibility', color: 'text-theme-yellow', border: 'border-theme-yellow/50' },
            { id: 'CPF Cancelado', icon: 'person_off', color: 'text-slate-600', border: 'border-slate-400/30' },
            { id: 'Preso', icon: 'lock', color: 'text-theme-navy', border: 'border-theme-navy/30' },
          ].map((s) => (
            <button
              key={s.id}
              onClick={() => setStatus(s.id as any)}
              className={`flex flex-col items-center justify-center gap-1 p-2 rounded-xl border transition-all ${
                status === s.id ? 'bg-white border-theme-navy shadow-md ring-1 ring-theme-navy' : 'bg-white/40 ' + s.border
              }`}
            >
              <span className={`material-symbols-outlined ${s.color} ${status === s.id ? 'fill-icon' : ''}`}>
                {s.icon}
              </span>
              <span className="text-[9px] font-bold text-theme-navy text-center leading-tight">
        ...
        <div className="mt-6 pmmg-card p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-theme-navy uppercase leading-none">Exibir no Mapa Tático</p>
              <p className="text-[10px] text-slate-500 mt-1">Marcar a última localização conhecida no mapa.</p>
            </div>
          </div>
          <button 
            onClick={() => setShowOnMap(!showOnMap)}
            className={`relative w-12 h-6 rounded-full transition-colors shrink-0 ${showOnMap ? 'bg-theme-navy' : 'bg-slate-300'}`}
          >
        ...
        <div className="flex items-center gap-2 mb-4 mt-8">
          <div className="h-4 w-1 bg-theme-navy rounded-full"></div>
          <h3 className="font-bold text-xs text-theme-navy uppercase tracking-wider">Dados Pessoais</h3>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">Nome Completo</label>
            <input 
              value={name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">CPF</label>
              <input 
                value={cpf}
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">RG (Registro Geral)</label>
              <input 
                value={rg}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">Alcunha / Vulgo</label>
              <input 
                value={nickname}
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">Data de Nascimento</label>
              <input 
                value={birthDate}
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">Descrição / Sinais Particulares</label>
            <textarea 
              value={description}
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">Nome da Mãe</label>
            <input 
              value={motherName}
        ...
        {/* 2. --- ARTIGOS CRIMINAIS --- */}
        <div className="flex items-center gap-2 mb-4 mt-8">
          <div className="h-4 w-1 bg-theme-red rounded-full"></div>
          <h3 className="font-bold text-xs text-theme-navy uppercase tracking-wider">Artigos Criminais</h3>
        </div>

        <div className="flex flex-wrap gap-2 mb-4">
          {articles.map((art, idx) => (
            <span key={idx} className="px-3 py-1.5 bg-theme-red/10 border border-theme-red/20 text-theme-red text-[10px] font-bold rounded-full flex items-center gap-1">
        ...
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 px-4 py-2 bg-white border border-theme-navy/20 rounded-lg text-xs" 
            placeholder="Ex: Art. 157" 
            type="text" 
          />
          <button 
            onClick={handleAddArticle}
            className="px-4 py-2 bg-theme-navy text-white text-[10px] font-bold rounded-lg uppercase"
          >
            Adicionar
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 mb-4 mt-8">
          <div className="h-4 w-1 bg-theme-navy rounded-full"></div>
          <h3 className="font-bold text-xs text-theme-navy uppercase tracking-wider">Endereço e Localização</h3>
        </div>
        <div className="space-y-4">
          
          {/* 3.1. Endereço Principal */}
          <div className="relative">
            <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">Endereço Principal</label>
            <div className="flex gap-2">
              <input 
              ...
              <button 
                ...
                className="bg-theme-blue text-white p-3 rounded-lg active:scale-95 transition-transform disabled:opacity-50 shrink-0"
              ...
              <button 
                ...
                className="bg-theme-navy text-white p-3 rounded-lg active:scale-95 transition-transform disabled:opacity-50 shrink-0"
            ...
            {lastSeenAddressSuggestions.length > 0 && (
              <div className="absolute z-10 w-full bg-white border border-theme-navy/20 rounded-lg mt-1 shadow-lg max-h-40 overflow-y-auto">
                {lastSeenAddressSuggestions.map((loc, index) => (
                  <button
                    key={index}
                    onClick={() => handleSelectLastSeenLocation(loc)}
                    className="w-full text-left px-4 py-2 text-sm text-theme-navy hover:bg-theme-khaki/50 transition-colors border-b border-theme-navy/5 last:border-b-0"
                  >
                    {loc.name}
                  </button>
                ))}
            ...
            {lastSeenAddressSuggestions.length === 0 && !isLastSeenSearching && lastSeenAddress.length > 2 && (
              <p className="text-[10px] text-theme-red italic text-center mt-2">Nenhum resultado encontrado.</p>
            )}
          </div>
          ...
          {selectedLastSeenLocation && (
            <div className="pmmg-card overflow-hidden">
              <div className="p-3 bg-theme-navy/5 flex items-center justify-between">
                <p className="text-[10px] font-bold text-theme-navy uppercase tracking-wider">Localização Confirmada (Endereço Principal)</p>
          ...
          <div className="relative">
            <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1 ml-1 tracking-wider">Endereço da Abordagem (Opcional)</label>
            <div className="flex gap-2">
              <input 
          ...
          {selectedApproachLocation && (
            <div className="pmmg-card overflow-hidden">
              <div className="p-3 bg-theme-navy/5 flex items-center justify-between">
                <p className="text-[10px] font-bold text-theme-navy uppercase tracking-wider">Localização Confirmada (Abordagem)</p>
                <span className="text-[9px] text-green-600 font-bold uppercase">GPS OK</span>
              </div>
              <GoogleMapWrapper
        ...
        <div className="flex items-center gap-2 mb-4 mt-8">
          <div className="h-4 w-1 bg-theme-navy rounded-full"></div>
          <h3 className="font-bold text-xs text-theme-navy uppercase tracking-wider">Veículos Cadastrados ({vehicles.length})</h3>
        </div>
        <div className="pmmg-card p-4 space-y-3">
          {vehicles.map((v, idx) => (
            <div key={idx} className="flex items-center justify-between p-2 bg-theme-navy/5 rounded-lg border border-theme-navy/10">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-theme-navy truncate">{v.plate} - {v.model}</p>
                <p className="text-[10px] text-slate-500">{v.color}</p>
              </div>
              <button onClick={() => handleRemoveVehicle(idx)} className="text-theme-red p-1 shrink-0">
                <span className="material-symbols-outlined text-lg">delete</span>
              </button>
            </div>
          ))}
          <div className="pt-2 space-y-2 border-t border-theme-navy/10">
            <div className="grid grid-cols-2 gap-3">
              <input 
                value={newVehiclePlate}
                onChange={(e) => setNewVehiclePlate(e.target.value)}
                className="w-full px-3 py-2 bg-white border border-theme-navy/20 rounded-lg text-xs" 
            ...
            <div className="flex gap-3">
              <input 
                ...
                className="flex-1 px-3 py-2 bg-white border border-theme-navy/20 rounded-lg text-xs" 
              ...
              <button 
                ...
                className="px-4 py-2 bg-theme-navy text-white text-[10px] font-bold rounded-lg uppercase disabled:opacity-50"
              >
                Adicionar
              </button>
            </div>
        ...
        <div className="flex items-center gap-2 mb-4 mt-8">
          <div className="h-4 w-1 bg-theme-navy rounded-full"></div>
          <h3 className="font-bold text-xs text-theme-navy uppercase tracking-wider">Ligações e Associações ({associations.length})</h3>
        </div>
        <div className="pmmg-card p-4 space-y-3">
              ...
              <div key={idx} className="flex items-center justify-between p-2 bg-theme-navy/5 rounded-lg border border-theme-navy/10">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] text-theme-navy/50 font-bold uppercase truncate">{a.relationship}</p>
                    <p className="text-sm font-bold text-theme-navy truncate">{associatedSuspect.name}</p>
                  </div>
                </div>
                <button onClick={() => handleRemoveAssociation(idx)} className="text-theme-red p-1 shrink-0">
                  <span className="material-symbols-outlined text-lg">delete</span>
                </button>
              </div>
            );
          ...
          <div className="pt-2 space-y-2 border-t border-theme-navy/10">
            ...
            {filteredSuspects.length > 0 && associationRelationship.trim() && (
              <div className="bg-white border border-theme-navy/20 rounded-lg shadow-md max-h-40 overflow-y-auto">
                {filteredSuspects.slice(0, 5).map((s) => (
                  <div 
                    key={s.id} 
                    className="flex items-center justify-between p-2 hover:bg-theme-khaki/50 transition-colors border-b border-theme-navy/5"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-theme-navy truncate">{s.name}</p>
                        <p className="text-[10px] text-slate-500">{s.cpf}</p>
                      </div>
                    </div>
                    <button 
                      onClick={() => handleAddAssociation(s.id)}
                      className="bg-theme-navy text-white text-[9px] font-bold px-3 py-1 rounded uppercase shrink-0"
                    >
                      Vincular
                    </button>
                  </div>
            ...
            {filteredSuspects.length > 0 && !associationRelationship.trim() && (
                <p className="text-[10px] text-theme-red italic text-center">Defina o tipo de ligação antes de vincular.</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-10 mb-8">
          <button 
            onClick={handleSave}
            className="w-full bg-theme-navy text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-3 uppercase tracking-widest active:scale-[0.98] transition-transform"
          >
