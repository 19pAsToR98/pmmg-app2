    ...
    <div className="flex flex-col h-full bg-theme-background overflow-hidden">
      <header className="sticky top-0 z-50 bg-theme-primary px-4 py-4 flex items-center justify-between shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-theme-accent tracking-wider uppercase mt-1">Criação e Convite</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto pb-32 no-scrollbar px-4 pt-4">
        <div className="pmmg-card p-4 space-y-4 mb-6">
          <h3 className="text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider">Detalhes do Grupo</h3>
          <div>
        ...
        <div className="pmmg-card p-4 space-y-4">
          <h3 className="text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider flex items-center justify-between">
            Convidar Oficiais 
            <span className="text-[9px] font-black text-theme-critical">({selectedMembers.length} Selecionado(s))</span>
          </h3>
              ...
              <div key={officer.id} className="flex items-center justify-between p-2 bg-theme-background/30 rounded-lg">
                ...
                <button 
                  ...
                  className={`text-[9px] font-bold px-3 py-1.5 rounded-lg uppercase active:scale-95 transition-transform ${
                    selectedMembers.includes(officer.id) ? 'bg-theme-critical text-white' : 'bg-theme-primary text-white'
                  }`}
                >
      ...
      <footer className="fixed bottom-0 left-0 right-0 p-4 bg-white/95 border-t border-theme-primary/10 backdrop-blur-lg max-w-md mx-auto">
        <button 
          ...
          className="w-full bg-theme-primary text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-3 uppercase tracking-widest active:scale-[0.98] transition-transform disabled:opacity-50"
        >
          <span className="material-symbols-outlined text-theme-accent">group_add</span>
          Criar Grupo Tático
        </button>
      </footer>
    </div>
