...
const ProfileSettings: React.FC<ProfileSettingsProps> = ({ navigateTo, onBack, currentRank, onRankChange, userAvatar }) => {
  return (
    <div className="flex flex-col h-full bg-theme-background overflow-hidden">
      <header className="bg-theme-primary px-6 pt-12 pb-8 rounded-b-[40px] shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-theme-accent/10 rounded-full -mr-16 -mt-16"></div>
        <div className="relative z-10 flex flex-col items-center">
          <div className="relative mb-4">
            <div className="w-28 h-28 rounded-full border-4 border-theme-accent/80 p-1 bg-white/10 overflow-hidden shadow-xl">
              <img alt={userAvatar.name} className="w-full h-full object-cover rounded-full grayscale" src={userAvatar.url} />
            </div>
            <div className="absolute -bottom-2 -right-2 p-1 bg-theme-primary rounded-lg border-2 border-theme-accent shadow-lg">
          ...
          <p className="text-theme-accent font-semibold text-[10px] tracking-[0.2em] uppercase mt-1">
            {currentRank} • 05º BPM
          </p>
        </div>
      </header>
      ...
      <main className="flex-1 px-4 -mt-6 pb-32 space-y-6 no-scrollbar overflow-y-auto">
        <section>
          <div className="flex items-center justify-between px-2 mb-3">
            <h3 className="text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider">Alterar Graduação</h3>
            <span className="text-[10px] font-black text-theme-critical bg-theme-critical/10 px-2 py-0.5 rounded uppercase">Identificação Tática</span>
          </div>
            ...
            <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar">
              {RANKS.map((rank) => (
                ...
                >
                  ...
                  <span className={`text-[9px] font-black uppercase ${currentRank === rank ? 'text-theme-accent' : 'text-theme-primary'}`}>
                    {rank}
                  </span>
                </button>
              ))}
        ...
        <section>
          <h3 className="px-2 mb-2 text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider">Identificação Profissional</h3>
          <div className="pmmg-card overflow-hidden">
            <div className="px-4">
              {[
                ...
                <div key={item.label} className={`flex items-center justify-between py-4 ${idx !== arr.length - 1 ? 'border-b border-theme-primary/5' : ''}`}>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-theme-primary/50 font-bold uppercase">{item.label}</span>
        ...
        <div className="flex flex-col items-center justify-center opacity-30 py-4">
          <span className="material-symbols-outlined text-theme-primary text-4xl mb-2">shield</span>
          <p className="text-[9px] font-bold uppercase tracking-widest text-theme-primary">PMMG • Versão 2.5.0</p>
        </div>
      </main>
