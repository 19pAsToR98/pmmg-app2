...
const TacticalContacts: React.FC<TacticalContactsProps> = ({ navigateTo, officers, contacts, onSendRequest, onAcceptRequest, onRejectRequest }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-theme-background overflow-hidden">
      <header className="sticky top-0 z-50 bg-theme-primary text-white shadow-xl px-4 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-theme-accent tracking-wider uppercase mt-1">Oficiais para Grupos Táticos</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto pb-32 no-scrollbar px-4 pt-4">
        ...
        <section className="mb-6">
          <h3 className="text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider mb-3">Buscar Oficial (Matrícula/Nome)</h3>
          ...
          {searchTerm.length > 0 && availableOfficers.length > 0 && (
            <div className="mt-3 pmmg-card p-3 space-y-2">
              {availableOfficers.map(officer => (
                <div key={officer.id} className="flex items-center justify-between p-2 bg-theme-background/30 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-theme-primary leading-tight">{officer.name}</p>
                      <p className="text-[10px] text-slate-500">{officer.rank} • {officer.unit}</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => onSendRequest(officer.id)}
                    className="bg-theme-primary text-white text-[9px] font-bold px-3 py-1.5 rounded-lg uppercase active:scale-95 transition-transform"
                  >
                    Solicitar Contato
                  </button>
                </div>
          ...
          {searchTerm.length > 0 && availableOfficers.length === 0 && (
            <p className="text-center text-[10px] text-theme-primary/50 mt-4">Nenhum oficial encontrado ou já é seu contato.</p>
          )}
        </section>

        {/* Contatos Aceitos */}
        <section className="mb-6">
          <h3 className="text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider mb-3 flex items-center gap-2">
          ...
          <div className="pmmg-card p-4 space-y-3">
            {acceptedContacts.length > 0 ? acceptedContacts.map(contact => {
              ...
              return (
                <div key={officer.id} className="flex items-center justify-between border-b border-theme-primary/5 pb-3 last:border-b-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-300">
                      <img src={officer.photoUrl} alt={officer.name} className="w-full h-full object-cover" />
                    </div>
        ...
        <section className="mb-6">
          <h3 className="text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider mb-3 flex items-center gap-2">
            ...
            {pendingRequests.length > 0 && (
              <span className="bg-theme-critical text-white text-[8px] font-bold px-2 py-0.5 rounded-full">{pendingRequests.length}</span>
            )}
          </h3>
          <div className="pmmg-card p-4 space-y-3">
                ...
                <div key={officer.id} className="flex items-center justify-between border-b border-theme-primary/5 pb-3 last:border-b-0 last:pb-0">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button 
                      ...
                      className="bg-theme-critical text-white p-2 rounded-lg active:scale-95 transition-transform"
                    >
        ...
        <section className="mb-6">
          <h3 className="text-[11px] font-bold text-theme-primary/60 uppercase tracking-wider mb-3">Solicitações Enviadas ({sentRequests.length})</h3>
          <div className="pmmg-card p-4 space-y-3">
            {sentRequests.length > 0 ? sentRequests.map(contact => {
              const officer = getOfficer(contact.officerId);
              if (!officer) return null;
              return (
                <div key={officer.id} className="flex items-center justify-between border-b border-theme-primary/5 pb-3 last:border-b-0 last:pb-0 opacity-70">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-slate-300">
                      <img src={officer.photoUrl} alt={officer.name} className="w-full h-full object-cover" />
                    </div>
                  ...
                  <span className="text-[10px] font-bold text-theme-accent uppercase">Pendente</span>
                </div>
              );
            }) : (
