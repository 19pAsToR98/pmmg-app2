...
const SuspectPhotoMarker = memo<{
  ...
  const getBorderColorClass = () => {
    switch (suspect.status) {
      case 'Foragido': return 'border-theme-red';
      case 'Suspeito': return 'border-theme-yellow';
      default: return 'border-theme-navy';
    }
  };
  ...
  const getBgColorClass = () => {
    switch (suspect.status) {
      case 'Foragido': return 'bg-theme-red';
      case 'Suspeito': return 'bg-theme-yellow';
      default: return 'bg-theme-navy';
    }
  };
  
  const getTextColorClass = () => {
    return suspect.status === 'Suspeito' ? 'text-theme-navy' : 'text-white';
  };
    ...
    >
      ...
      >
        {usePhotoMarker ? (
          ...
          // ✅ ÍCONE SIMPLES (24x24)
          <div 
            className={`w-6 h-6 rounded-full border-2 border-white flex items-center justify-center shadow-md ${getBgColorClass()}`}
            title={suspect.name}
          >
            <span className={`material-symbols-outlined text-[14px] fill-icon ${getTextColorClass()}`}>
              {getIconName()}
            </span>
          </div>
        )}
...
const UserMarkerComponent = memo<{
  ...
  return (
    ...
    >
      ...
      >
        <div 
          className="w-8 h-8 bg-theme-blue rounded-full border-4 border-white flex items-center justify-center shadow-lg ring-2 ring-theme-blue/50"
          title="Você (Oficial)"
        >
...
const TacticalMap: React.FC<TacticalMapProps> = ({ navigateTo, suspects, onOpenProfile, initialCenter, customMarkers, addCustomMarker, updateCustomMarker, deleteCustomMarker }) => {
  ...
  const STATUS_FILTERS: { id: MapFilter, label: string, color: string, icon: string }[] = [
    { id: 'Foragido', label: 'Foragido', color: 'bg-theme-red', icon: 'priority_high' },
    { id: 'Suspeito', label: 'Suspeito', color: 'bg-theme-yellow', icon: 'warning' },
    { id: 'Preso', label: 'Preso', color: 'bg-theme-blue', icon: 'lock' },
  ...
  return (
    <div className="flex flex-col h-full bg-theme-khaki overflow-hidden">
      <header className="sticky top-0 z-[1000] bg-theme-navy px-4 py-4 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 shrink-0 bg-white rounded-full flex items-center justify-center p-1 border-2 border-theme-red">
              <span className="material-symbols-outlined text-theme-navy text-xl">map</span>
            </div>
            <div>
              <h1 className="font-bold text-xs leading-none text-white uppercase tracking-tight">Mapa Tático</h1>
              <p className="text-[9px] font-medium text-theme-yellow tracking-wider uppercase mt-1">Inteligência Territorial</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button 
              ...
              className={`p-2 rounded-full border transition-all ${isAddingMarker ? 'bg-theme-red text-white border-theme-red shadow-lg' : 'bg-white/10 text-white border-white/20'}`}
            >
      ...
      <div className="flex-1 relative">
            ...
            <>
              ...
              {activeInfoWindow === 'user-pos' && (
                <InfoWindowF position={userPos} onCloseClick={() => setActiveInfoWindow(null)}>
                  <div className="p-2">
                    <p className="font-bold text-theme-navy text-sm">Você (Oficial)</p>
                    ...
                    <InfoWindowF position={position} onCloseClick={() => setActiveInfoWindow(null)}>
                      <div className="p-2 min-w-[150px]">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <div>
                            <p className="font-bold text-[10px] text-theme-navy uppercase leading-tight">{suspect.name}</p>
                            <p className="text-[9px] text-theme-blue font-bold uppercase">{locationType}</p>
                        ...
                        <div className="flex gap-2 mt-3">
                          <button 
                            ...
                            className="flex-1 bg-theme-navy text-white text-[9px] font-bold py-1.5 rounded uppercase tracking-wider"
                          >
                            Ver Ficha
                          </button>
                          <button 
                            onClick={() => handleShareLocation(lat, lng, `${locationType} de ${suspect.name}`)} 
                            className="px-3 border-2 border-theme-navy/20 rounded-lg flex items-center justify-center"
                          >
                            <span className="material-symbols-outlined text-theme-navy text-lg">share</span>
                          </button>
                        </div>
                      </div>
                    </InfoWindowF>
            ...
            return (
              <React.Fragment key={markerId}>
                ...
                {activeInfoWindow === markerId && (
                  <InfoWindowF position={position} onCloseClick={() => setActiveInfoWindow(null)}>
                    <div className="p-2 min-w-[150px]">
                      <p className="font-bold text-[11px] text-theme-navy uppercase leading-tight">{markerData.title}</p>
                      ...
                      <div className="flex gap-2 mt-3">
                        <button 
                          ...
                          className="flex-1 bg-theme-navy text-white text-[9px] font-bold py-1.5 rounded uppercase tracking-wider flex items-center justify-center"
                        >
                          <span className="material-symbols-outlined text-sm">edit</span>
                        </button>
                        <button 
                          onClick={() => handleShareLocation(markerData.lat, markerData.lng, markerData.title)}
                          className="flex-1 bg-theme-blue text-white text-[9px] font-bold py-1.5 rounded uppercase tracking-wider flex items-center justify-center"
                        >
                          <span className="material-symbols-outlined text-sm">share</span>
                        </button>
                        <button 
                          onClick={() => handleDeleteMarker(markerData.id)}
                          className="px-3 bg-theme-red text-white text-[9px] font-bold py-1.5 rounded uppercase tracking-wider flex items-center justify-center"
                        >
              ...
              icon={{
                url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
                  <svg width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="20" cy="20" r="18" fill="var(--color-red)" stroke="#ffffff" stroke-width="4"/>
        ...
        {activeMarkerData && (
          <div className="absolute inset-0 z-[1002] bg-black/50 flex items-center justify-center p-4">
            <div className="bg-white p-5 rounded-xl shadow-2xl w-full max-w-sm">
              <h3 className="text-lg font-bold text-theme-navy uppercase mb-4 border-b pb-2">
                {isEditing ? 'Editar Marcador Tático' : 'Configurar Novo Marcador'}
              </h3>
              
              <div className="space-y-3">
                <div>
                  <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1">Título</label>
                  <input 
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1">Descrição</label>
                  <textarea 
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1">Ícone ({activeMarkerData.icon})</label>
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {['flag', 'warning', 'shield', 'camera', 'bolt', 'visibility'].map(icon => (
                      <button 
                        ...
                        className={`p-2 rounded-lg border transition-all ${activeMarkerData.icon === icon ? 'bg-theme-navy text-theme-yellow border-theme-yellow' : 'bg-slate-100 text-theme-navy/50'}`}
                      >
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase text-theme-navy/70 mb-1">Cor</label>
                  <div className="flex gap-2">
                    {[{c: 'bg-theme-gold', v: 'theme-gold'}, {c: 'bg-theme-red', v: 'theme-red'}, {c: 'bg-theme-blue', v: 'theme-blue'}, {c: 'bg-green-500', v: 'green-500'}].map(color => (
                      <button 
                        key={color.v}
                        onClick={() => {
                        ...
                        className={`w-8 h-8 rounded-full border-2 transition-all ${color.c} ${activeMarkerData.color === color.c ? 'ring-4 ring-offset-2 ring-theme-navy' : 'border-white'}`}
                      ></button>
                    ))}
                  </div>
                </div>
        ...
        <div className={`absolute top-4 right-0 z-[1000] bottom-[100px] transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <button 
            ...
            className="absolute left-0 top-1/2 transform -translate-x-full -translate-y-1/2 bg-theme-navy p-1.5 rounded-l-xl shadow-xl text-theme-yellow"
          >
          ...
          {/* Conteúdo do Painel */}
          <div className="bg-white/95 backdrop-blur-md p-3 rounded-l-2xl shadow-2xl border border-theme-navy/10 flex flex-col gap-2.5 h-full overflow-y-auto w-64">
            <p className="text-[8px] font-black text-theme-navy/40 uppercase tracking-widest border-b border-theme-navy/5 pb-1 mb-1">Legenda Tática</p>
            
            {/* --- Filtro de Localização --- */}
            <div className="pt-2 pb-3 border-b border-theme-navy/5">
              <p className="text-[9px] font-black text-theme-navy/60 uppercase tracking-wider mb-2">Tipo de Localização</p>
              <div className="flex gap-2">
                <button 
                  onClick={() => setLocationFilter('residence')}
                  className={`flex-1 text-[9px] font-bold uppercase py-1.5 rounded-lg transition-all ${locationFilter === 'residence' ? 'bg-theme-navy text-white shadow-md' : 'bg-slate-100 text-theme-navy/70'}`}
                >
                  <span className="material-symbols-outlined text-sm mr-1">location_on</span> Residência
                </button>
                <button 
                  onClick={() => setLocationFilter('approach')}
                  className={`flex-1 text-[9px] font-bold uppercase py-1.5 rounded-lg transition-all ${locationFilter === 'approach' ? 'bg-theme-navy text-white shadow-md' : 'bg-slate-100 text-theme-navy/70'}`}
                >
            ...
            {/* --- Filtro de Status --- */}
            <p className="text-[8px] font-black text-theme-navy/40 uppercase tracking-widest border-b border-theme-navy/5 pb-1 mb-1 pt-2">Filtro por Status</p>

            <button 
              onClick={() => setActiveFilter('Todos')}
              className={`flex items-center gap-2 w-full text-left p-1 rounded transition-colors ${activeFilter === 'Todos' ? 'bg-theme-navy/10' : 'hover:bg-slate-50'}`}
            >
               <div className={`w-4 h-4 rounded-full bg-theme-navy flex items-center justify-center shadow-sm`}>
                 <span className="material-symbols-outlined text-white text-[10px] fill-icon">done_all</span>
               </div>
               <span className="text-[9px] font-bold text-theme-navy uppercase">Todos os Suspeitos ({suspects.length})</span>
            </button>
            ...
            {STATUS_FILTERS.map(filter => (
              <button 
                ...
                className={`flex items-center gap-2 w-full text-left p-1 rounded transition-colors ${activeFilter === filter.id ? 'bg-theme-navy/10' : 'hover:bg-slate-50'}`}
              >
                <div className={`w-4 h-4 ${usePhotoMarker ? 'rounded-md border-2 bg-slate-300' : 'rounded-full flex items-center justify-center'} ${filter.color} border-white shadow-sm`}>
                  {!usePhotoMarker && <span className={`material-symbols-outlined text-[10px] fill-icon ${filter.id === 'Suspeito' ? 'text-theme-navy' : 'text-white'}`}>{filter.icon}</span>}
                </div>
                <span className="text-[9px] font-bold text-theme-navy uppercase">{filter.label}</span>
              </button>
            ))}
            
            {/* Oficial */}
            <div className="flex items-center gap-2 pt-2 border-t border-theme-navy/5">
               <div className="w-3.5 h-3.5 bg-theme-blue rounded-full border-2 border-white shadow-sm ring-1 ring-theme-blue/50"></div>
               <span className="text-[9px] font-bold text-theme-navy uppercase">Oficial (Você)</span>
            </div>
            
            {/* Ponto Tático */}
            <div className="flex items-center gap-2">
               <div className="w-3.5 h-3.5 bg-theme-gold rounded-full border-2 border-white shadow-sm ring-1 ring-theme-gold/30"></div>
               <span className="text-[9px] font-bold text-theme-navy uppercase">Ponto Tático</span>
            </div>
            
            {activeFilter !== 'Todos' && (
              <button 
                onClick={() => setActiveFilter('Todos')}
                className="mt-2 text-[8px] font-black text-theme-red uppercase border-t border-theme-navy/5 pt-2 text-left"
              >
            ...
            <div className="mt-4 pt-4 border-t border-theme-navy/5">
              <p className="text-[8px] font-black text-theme-navy/40 uppercase tracking-widest mb-2">Visualização de Camadas</p>
              
              <button 
                onClick={() => setMapType('roadmap')}
                className={`w-full text-left text-[10px] font-bold uppercase py-1.5 px-2 rounded transition-colors flex items-center gap-1 ${mapType === 'roadmap' ? 'bg-theme-navy text-white' : 'text-theme-navy/70 hover:bg-theme-navy/5'}`}
              >
              ...
              <button 
                ...
                className={`w-full text-left text-[10px] font-bold uppercase py-1.5 px-2 rounded transition-colors flex items-center gap-1 ${mapType === 'satellite' ? 'bg-theme-navy text-white' : 'text-theme-navy/70 hover:bg-theme-navy/5'}`}
              >
              ...
              <button 
                ...
                className={`w-full text-left text-[10px] font-bold uppercase py-1.5 px-2 rounded transition-colors flex items-center gap-1 ${mapType === 'hybrid' ? 'bg-theme-navy text-white' : 'text-theme-navy/70 hover:bg-theme-navy/5'}`}
              >
