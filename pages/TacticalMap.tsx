...
const TacticalMap: React.FC<TacticalMapProps> = ({ navigateTo, suspects, onOpenProfile, initialCenter, customMarkers, addCustomMarker, updateCustomMarker, deleteCustomMarker }) => {
  ...
  // REMOVIDO: handleMapClick não é mais necessário
  ...
  // 2. Gerenciamento do Modo de Posicionamento (Placement Mode)
  useEffect(() => {
    const map = mapInstanceRef.current;
    if (!map) return;

    if (isPlacementMode && newMarkerData) {
      // 1. Create or update temporary marker
      const tempIcon = L.divIcon({
        className: 'custom-temp-marker-icon',
        html: `<div class="w-10 h-10 bg-pmmg-red rounded-full border-4 border-white flex items-center justify-center shadow-xl animate-bounce"><span class="material-symbols-outlined text-white text-[20px] fill-icon">pin_drop</span></div>`,
        iconSize: [40, 40],
        iconAnchor: [20, 40] // Anchor no centro inferior para efeito de pin
      });

      if (!tempMarkerRef.current) {
        tempMarkerRef.current = L.marker([newMarkerData.lat, newMarkerData.lng], { icon: tempIcon }).addTo(map);
      } else {
        tempMarkerRef.current.setLatLng([newMarkerData.lat, newMarkerData.lng]);
        tempMarkerRef.current.setIcon(tempIcon);
      }

      // 2. Add map listener to update coordinates in the modal/state
      const handleMapMove = () => {
        const center = map.getCenter();
        // Atualiza newMarkerData com as coordenadas do centro do mapa
        ...
        // Move o marcador temporário para seguir o centro do mapa
        tempMarkerRef.current?.setLatLng(center);
      };
      ...
      // Garante que o marcador temporário seja removido ao sair do modo
      if (tempMarkerRef.current) {
        tempMarkerRef.current.remove();
        tempMarkerRef.current = null;
      }
  ...
  return (
    <div className="flex flex-col h-full bg-pmmg-khaki overflow-hidden">
      ...
      <div className="flex-1 relative">
        ...
        {/* REMOVIDO: Adding Marker Mode Indicator (Non-blocking) - Substituído pelo marcador temporário do Leaflet */}
              ...
              <div className="space-y-3">
                {isPlacementMode && (
                  <div className="bg-pmmg-navy/5 p-3 rounded-lg border border-pmmg-navy/10">
                    <p className="text-[10px] font-bold uppercase text-pmmg-navy/70 mb-1">Coordenadas Atuais (Arraste o Mapa)</p>
