...
const Dashboard: React.FC<DashboardProps> = ({ navigateTo, navigateToSuspectsManagement, onOpenProfile, suspects }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-theme-background overflow-hidden">
      <header className="sticky top-0 z-50 bg-theme-primary px-4 pt-4 pb-4 shadow-xl">
        {/* Top Bar */}
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 shrink-0 bg-white rounded-full flex items-center justify-center p-1 border-2 border-theme-critical shadow-inner">
            ...
            <div>
              ...
              <p className="text-[10px] font-medium text-theme-accent tracking-wider uppercase mt-1">Quartel General</p>
            </div>
          </div>
        ...
        <div className="relative group">
          <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
            <span className="material-symbols-outlined text-theme-primary text-xl">search</span>
          </div>
          <input 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="block w-full pl-10 pr-4 py-3 bg-white rounded-xl border-2 border-theme-primary/20 focus:border-theme-primary focus:ring-0 text-sm font-bold placeholder-theme-primary/40 shadow-sm" 
      ...
      <main className="flex-1 overflow-y-auto pb-32 no-scrollbar">
        ...
        <section className="px-4 pt-4 grid grid-cols-2 gap-3">
          <button onClick={() => handleCardClick('Foragido')} className="pmmg-card p-3 border-l-4 border-l-theme-critical active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-primary/60 block mb-1">Foragidos</span>
            <div className="flex items-end justify-between">
              <span className="text-2xl font-black text-theme-critical">{stats.foragidos}</span>
              <span className="material-symbols-outlined text-theme-critical/30">release_alert</span>
            </div>
          </button>
          <button onClick={() => handleCardClick('Suspeito')} className="pmmg-card p-3 border-l-4 border-l-theme-accent active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-primary/60 block mb-1">Suspeitos</span>
            <div className="flex items-end justify-between">
              <span className="text-2xl font-black text-theme-primary">{stats.suspeitos}</span>
              <span className="material-symbols-outlined text-theme-accent">visibility</span>
            </div>
          </button>
          <button onClick={() => handleCardClick('Preso')} className="pmmg-card p-3 border-l-4 border-l-theme-info active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-primary/60 block mb-1">Presos</span>
            <div className="flex items-end justify-between">
              <span className="text-2xl font-black text-theme-info">{stats.presos}</span>
              <span className="material-symbols-outlined text-theme-info/30">lock</span>
            </div>
          </button>
          <button onClick={() => handleCardClick('CPF Cancelado')} className="pmmg-card p-3 border-l-4 border-l-slate-600 active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-primary/60 block mb-1">CPF Cancelado</span>
            <div className="flex items-end justify-between">
        ...
        <section className="px-4 pt-4">
          <button 
            ...
            className="w-full bg-theme-primary text-white text-xs font-bold py-3 rounded-xl uppercase tracking-widest flex items-center justify-center gap-2 active:scale-[0.98] transition-transform border border-theme-accent/30"
          >
            <span className="material-symbols-outlined text-lg text-theme-accent">person_add</span>
            Novo Registro
          </button>
        </section>
        ...
        <div className="px-4 pt-8 pb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="h-4 w-1.5 bg-theme-critical rounded-full"></div>
            <h3 className="font-bold text-xs text-theme-primary uppercase tracking-widest italic">
              ...
              <div className="flex">
                <div className="w-32 h-44 relative bg-slate-200 shrink-0">
                  ...
                  <div className={`absolute top-0 left-0 text-white text-[8px] font-bold px-2 py-1 uppercase rounded-br-lg shadow-md ${
                    alert.status === 'Foragido' ? 'bg-theme-critical' : 
                    alert.status === 'Suspeito' ? 'bg-theme-accent text-theme-primary' :
                    alert.status === 'Preso' ? 'bg-theme-info' : 'bg-slate-700'
                  }`}>
                    {alert.status}
                  </div>
                </div>
                <div className="flex-1 p-3 flex flex-col justify-between overflow-hidden">
                  <div>
                    <div className="flex justify-between items-start">
                      <h4 className="font-bold text-sm text-theme-primary uppercase leading-tight truncate pr-1">{alert.name}</h4>
                      <span className={`material-symbols-outlined fill-icon text-lg ${
                        alert.status === 'Foragido' ? 'text-theme-critical' : 'text-theme-accent'
                      }`}>
                    ...
                    <div className="mt-2 space-y-1">
                      <div className="flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[14px] text-theme-primary">location_on</span>
                        <span className="text-[10px] text-slate-700 truncate">Visto em: {alert.lastSeen}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[14px] text-theme-primary">history</span>
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 bg-theme-primary text-white text-[9px] font-bold py-2 rounded-lg uppercase tracking-wide"
                    >
                      Ficha Completa
                    </button>
                    <button 
