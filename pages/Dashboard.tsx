...
const Dashboard: React.FC<DashboardProps> = ({ navigateTo, navigateToSuspectsManagement, onOpenProfile, suspects }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-theme-khaki overflow-hidden">
      <header className="sticky top-0 z-50 bg-theme-navy px-4 pt-4 pb-4 shadow-xl">
        {/* Top Bar */}
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 shrink-0 bg-white rounded-full flex items-center justify-center p-1 border-2 border-theme-red shadow-inner">
              <span className="material-symbols-outlined text-theme-navy text-2xl">shield</span>
            </div>
            <div>
              <h1 className="font-bold text-sm leading-none text-white uppercase tracking-tight">PMMG OPERACIONAL</h1>
              <p className="text-[10px] font-medium text-theme-yellow tracking-wider uppercase mt-1">Quartel General</p>
            </div>
          </div>
        ...
        <div className="relative group">
          <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none">
            <span className="material-symbols-outlined text-theme-navy text-xl">search</span>
          </div>
          <input 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="block w-full pl-10 pr-4 py-3 bg-white rounded-xl border-2 border-theme-navy/20 focus:border-theme-navy focus:ring-0 text-sm font-bold placeholder-theme-navy/40 shadow-sm" 
      ...
      <main className="flex-1 overflow-y-auto pb-32 no-scrollbar">
        ...
        <section className="px-4 pt-4 grid grid-cols-2 gap-3">
          <button onClick={() => handleCardClick('Foragido')} className="pmmg-card p-3 border-l-4 border-l-theme-red active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-navy/60 block mb-1">Foragidos</span>
            <div className="flex items-end justify-between">
              <span className="text-2xl font-black text-theme-red">{stats.foragidos}</span>
              <span className="material-symbols-outlined text-theme-red/30">release_alert</span>
            </div>
          </button>
          <button onClick={() => handleCardClick('Suspeito')} className="pmmg-card p-3 border-l-4 border-l-theme-yellow active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-navy/60 block mb-1">Suspeitos</span>
            <div className="flex items-end justify-between">
              <span className="text-2xl font-black text-theme-navy">{stats.suspeitos}</span>
              <span className="material-symbols-outlined text-theme-yellow">visibility</span>
            </div>
          </button>
          <button onClick={() => handleCardClick('Preso')} className="pmmg-card p-3 border-l-4 border-l-theme-blue active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-navy/60 block mb-1">Presos</span>
            <div className="flex items-end justify-between">
              <span className="text-2xl font-black text-theme-blue">{stats.presos}</span>
              <span className="material-symbols-outlined text-theme-blue/30">lock</span>
            </div>
          </button>
          <button onClick={() => handleCardClick('CPF Cancelado')} className="pmmg-card p-3 border-l-4 border-l-slate-600 active:scale-[0.98] transition-transform text-left">
            <span className="text-[10px] font-bold uppercase text-theme-navy/60 block mb-1">CPF Cancelado</span>
            <div className="flex items-end justify-between">
        ...
        <section className="px-4 pt-4">
          <button 
            ...
            className="w-full bg-theme-navy text-white text-xs font-bold py-3 rounded-xl uppercase tracking-widest flex items-center justify-center gap-2 active:scale-[0.98] transition-transform border border-theme-yellow/30"
          >
            <span className="material-symbols-outlined text-lg text-theme-yellow">person_add</span>
            Novo Registro
          </button>
        </section>
        ...
        <div className="px-4 pt-8 pb-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-xs text-theme-navy uppercase tracking-widest italic">
              ...
              <div className="flex">
                <div className="w-32 h-44 relative bg-slate-200 shrink-0">
                  ...
                  <div className={`absolute top-0 left-0 text-white text-[8px] font-bold px-2 py-1 uppercase rounded-br-lg shadow-md ${
                    alert.status === 'Foragido' ? 'bg-theme-red' : 
                    alert.status === 'Suspeito' ? 'bg-theme-yellow text-theme-navy' :
                    alert.status === 'Preso' ? 'bg-theme-blue' : 'bg-slate-700'
                  }`}>
                    {alert.status}
                  </div>
                </div>
                <div className="flex-1 p-3 flex flex-col justify-between overflow-hidden">
                  <div>
                    <div className="flex justify-between items-start">
                      <h4 className="font-bold text-sm text-theme-navy uppercase leading-tight truncate pr-1">{alert.name}</h4>
                      <span className={`material-symbols-outlined fill-icon text-lg ${
                        alert.status === 'Foragido' ? 'text-theme-red' : 'text-theme-yellow'
                      }`}>
                    ...
                    <div className="mt-2 space-y-1">
                      <div className="flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[14px] text-theme-navy">location_on</span>
                        <span className="text-[10px] text-slate-700 truncate">Visto em: {alert.lastSeen}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[14px] text-theme-navy">history</span>
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className="flex-1 bg-theme-navy text-white text-[9px] font-bold py-2 rounded-lg uppercase tracking-wide"
                    >
                      Ficha Completa
                    </button>
                    <button 
