...
const Dashboard: React.FC<DashboardProps> = ({ navigateTo, navigateToSuspectsManagement, onOpenProfile, suspects, startShareFlow }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-pmmg-khaki dark:bg-slate-900 overflow-hidden">
      <header className="sticky top-0 z-50 bg-pmmg-navy px-4 pt-4 pb-4 shadow-xl shrink-0">
        {/* Top Bar */}
      ...
      <main className="flex-1 overflow-y-auto lg:pb-4 no-scrollbar">
        <div className="lg:grid lg:grid-cols-3 lg:gap-6 lg:p-6">
          {/* Coluna 1: Stats e Novo Registro */}
          <div className="lg:col-span-1">
            {/* Stats Grid */}
            <section className="px-4 pt-4 lg:p-0 grid grid-cols-2 gap-3">
            ...
            {/* New Registry Button */}
            <section className="px-4 pt-4 lg:p-0 lg:mt-4">
              <button 
          ...
          {/* Coluna 2/3: Recent Alerts (Expanded on Desktop) */}
          <div className="lg:col-span-2">
            {/* Recent Alerts Header */}
            <div className="px-4 pt-8 pb-3 lg:pt-0 flex items-center justify-between">
              <div className="flex items-center gap-2">
            ...
            <section className="px-4 lg:p-0 space-y-4">
              {recentSuspects.length > 0 ? recentSuspects.map((s) => (
                <div 
                  key={s.id} 
                  onClick={() => onOpenProfile(s.id)}
                  className="pmmg-card overflow-hidden transition-all active:scale-[0.98] cursor-pointer lg:flex"
                >
                  <div className="flex">
