...
const SuspectListItem: React.FC<SuspectListItemProps> = ({
  ...
  const statusColor = s.status === 'Foragido' ? 'bg-theme-red' : 
                        s.status === 'Suspeito' ? 'bg-theme-yellow text-theme-navy' :
                        s.status === 'Preso' ? 'bg-theme-blue' : 'bg-slate-700';
  ...
  return (
    <div 
      ...
      {...longPressProps}
      className={`pmmg-card flex p-3 gap-4 items-center transition-all shadow-sm border-l-4 border-l-theme-navy relative ${
        isSelectionMode ? (isSelected ? 'bg-theme-red/10 border-l-theme-red ring-1 ring-theme-red' : 'opacity-70') : 'active:scale-[0.98]'
      }`}
    >
      {isSelectionMode && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-0.5">
           <h3 className="text-[11px] font-black text-theme-navy uppercase truncate pr-2">{s.name}</h3>
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {s.nickname && <span className="text-[9px] font-black text-theme-red italic">"{s.nickname}"</span>}
        </div>
        <div className="mt-1.5 flex flex-wrap gap-2">
           {primaryVehicle && (
             <div className="flex items-center gap-1 text-[8px] bg-white px-2 py-0.5 rounded border border-theme-navy/5 text-slate-600 font-bold">
      ...
      <span className="material-symbols-outlined text-theme-navy/20 shrink-0">chevron_right</span>
    </div>
  );
};
