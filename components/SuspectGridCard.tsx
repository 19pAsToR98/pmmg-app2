...
const SuspectGridCard: React.FC<SuspectGridCardProps> = ({ suspect }) => {
  const statusColor = suspect.status === 'Foragido' ? 'bg-theme-red' : 
                        suspect.status === 'Suspeito' ? 'bg-theme-yellow text-theme-navy' :
                        suspect.status === 'Preso' ? 'bg-theme-blue' : 'bg-slate-700';
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-3 flex flex-col justify-between">
        <div>
          <h4 className="font-bold text-xs text-theme-navy uppercase leading-tight truncate">{suspect.name}</h4>
        ...
        <div className="mt-2">
          <button 
            ...
            className="w-full bg-theme-navy text-white text-[8px] font-bold py-1.5 rounded-lg uppercase tracking-wide"
          >
            Ver Ficha
          </button>
        </div>
