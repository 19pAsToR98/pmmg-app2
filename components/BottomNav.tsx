...
const BottomNav: React.FC<BottomNavProps> = ({ activeScreen, navigateTo }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/95 border-t border-theme-navy/10 px-6 pt-3 pb-8 z-50 backdrop-blur-lg max-w-md mx-auto">
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 transition-colors ${
              activeScreen === tab.id ? 'text-theme-navy' : 'text-theme-navy/40'
            }`}
          >
            <div className="relative">
              ...
              {('badge' in tab) && tab.badge && tab.badge > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-theme-red text-white text-[8px] flex items-center justify-center rounded-full border border-white">
                  {tab.badge}
                </span>
              )}
            </div>
